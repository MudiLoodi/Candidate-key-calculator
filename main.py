...
def main():
    try:
        # Get input
        relation = input("Enter the relation (comma-separated attributes): ")
        dependencies = input("Enter the functional dependencies (e.g., A->B,B->C): ")

        # Process input
        attributes = [attr.strip() for attr in relation.split(',')]
        dependency_list = dependencies.replace(' ', '').split(',')

        # Analyze relation
        analyzer = RelationAnalyzer(attributes, dependency_list)
        keys = analyzer.find_candidate_keys()

        # Print results
        print("\n" + "-"*50)
        print(f"Attributes: {attributes}")
        print(f"Left side only: {analyzer.categories.left}")
        print(f"Right side only: {analyzer.categories.right}")
        print(f"Neither side: {analyzer.categories.neither}")
        print(f"Both sides: {analyzer.categories.both}")
        print(f"Candidate Key(s): {keys}")
        print("-"*50 + "\n")
